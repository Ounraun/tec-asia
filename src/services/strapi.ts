...
import { StrapiCollection, StrapiResponse, BookingAttrs, BookingFlat, normalizeBooking } from '../types/booking';
...
export async function getBookingsByWeek(roomDocumentId: string, monday: Date, sunday: Date): Promise<BookingFlat[]> {
  ...
  const url =
    ...
    (roomDocumentId ? `&filters[meeting_room][documentId][$eq]=${roomDocumentId}` : '') +
    `&populate=*`;
...
export async function createBooking(p: CreateBookingPayload): Promise<BookingFlat> {
...
export async function updateBooking(
  ...
  payload: {
    ...
    participants?: string[]; meeting_roomDocumentId?: string;
  },
  opts?: { requestId?: string }
) {
  const rid = opts?.requestId || `fe_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,8)}`;
  const body = {
    data: {
      locale: 'en',
      ...
      meeting_room: payload.meeting_roomDocumentId
        ? { connect: [payload.meeting_roomDocumentId] }
        : undefined,
      email: (payload.participants || []).map((e) => ({ email: e })),
    },
  };

  const res = await fetch(`${import.meta.env.VITE_API_URL}/api/bookings/${docId}?locale=en`, {
    method: 'PUT',
    headers: {
...
export async function getBookingByDocId(documentId: string): Promise<BookingFlat> {
